...
class AdminOrMe:
    ...
    def __call__(
        self, user: common.authentication.authenticated_user, request: fastapi.Request
    ):
        """
        ...
        """
        if (
            int(request.path_params.get(self.identifier_variable)) == user.id
            or common.constants.ADMIN_ROLE_ID in user.roles
        ):
            return user
