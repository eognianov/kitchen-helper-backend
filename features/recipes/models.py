...
class RecipeCategory(DbBaseModel):
    ...
    recipes: Mapped[list["Recipe"]] = relationship(
        "Recipe", back_populates="category", init=False, lazy="selectin"
    )
...
class Recipe(DbBaseModel):
    ...
    category_id: Mapped[int] = mapped_column(
        ForeignKey("RECIPE_CATEGORIES.id"), nullable=True, default=None
    )
    ...
    cholesterol: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, default=0
    )
    is_published: Mapped[bool] = mapped_column(Boolean, default=False)
    published_on: Mapped[datetime.datetime] = mapped_column(
