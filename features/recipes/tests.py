...
class TestRecipe:
    ...
    def test_update(self, use_test_db, bypass_published_filter, mocker):
        ...
        with db.connection.get_session() as session:
            recipes = session.query(Recipe).all()
            updated_recipe = session.query(Recipe).first()
    ...
    # def test_update_recipe_success(self, use_test_db, bypass_published_filter, mocker):
    #     operations.create_category("Category", 1)
    #     operations.create_recipe(**self.recipe)
    #     recipe_id = 1
    #
    #     with db.connection.get_session() as session:
    #         recipes = session.query(Recipe).all()
    #         original_recipe = session.query(Recipe).first()
    #
    #     assert len(recipes) == 1
