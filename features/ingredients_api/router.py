...
def create_ingredient_category(created_category_input_model: IngredientCategory) -> IngredientCategory:
    ...
    try:
        return features.ingredients_api.operations.create_ingredient_category(created_category_input_model.name, created_category_input_model.created_by)
...
def update_ingredient_category(category_id: int = fastapi.Path(), patch_ingredient_category_input_model: PatchIngredientCategoryInputModel = fastapi.Body()) -> IngredientCategory:
    ...
    try:
        return features.ingredients_api.operations.update_ingredient_category(category_id, **patch_ingredient_category_input_model.model_dump())
...
def delete_category(category_id: int = fastapi.Path()):
    ...
    try:
        return features.ingredients_api.operations.get_ingredient_category_by_id(category_id=category_id)
    except features.ingredients_api.exceptions.IngredientCategoryNotFoundException:
        raise fastapi.HTTPException(
            status_code=fastapi.status.HTTP_400_BAD_REQUEST,
...
def delete_ingredient(ingredient_id: int = fastapi.Path()):
    ...
    try:
        return features.ingredients_api.operations.delete_ingredient(ingredient_id=ingredient_id)
